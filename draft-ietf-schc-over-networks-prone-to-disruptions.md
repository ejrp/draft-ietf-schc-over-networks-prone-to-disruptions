...
stand_alone: true
ipr: trust200902
docname: draft-ietf-schc-over-networks-prone-to-disruptions-02
area: Internet
...
abbrev: SCHC over networks prone to disruptions
wg: SCHC Working Group
author:
- ins: E. Ramos
  name: Edgar Ramos
...
- ins: R. Munoz-Lara
  ...
  org: Departamento de Ingenieria Electrica, Universidad de Chile
  city: Av. Tupper 2007, Santiago
  country: Chile
  email: 	rmunozlara@ing.uchile.cl
- ins: S. Cespedes
  name: Sandra Cespedes
  org: Concordia University
  city: 1455 De Maisonneuve Blvd. W., Montreal QC, H3G 1M8
  country: Canada
...
## Direct-to-satellite IoT (DtS-IoT) devices
...
In the figure {{fig_dtsiot-topo1}}, the end device communicates bidirectionally with the Ground Station via a LEO satellite.
...
~~~~~~
{: #fig_dtsiot-topo1 title='Topology DtS-IoT.'}


### Disruptions in DtS-IoT
...
Due to the asynchronous communication provided by the two time windows, the **transfer delay** of a SCHC packet increases. Figure {{fig_dtsiot-flow_mesg}} shows the message flow for sending a SCHC window with its corresponding acknowledgement through a DtS-IoT environment. Note that the increase in the transfer delay of a SCHC packet is directly related to the revisit time, since the fragmenter (end-device) waits for a SCHC ACK when an SCHC window has been completely transmitted or when an ACK REQ has been sent, as defined in {{!RFC8724}}.
   ...
   Revisit |  |                   |--- W=0,FCN=2 ---->|            
      Time |  |                   |--- W=0,FCN=1 ---->|            
           ...
           v  |+-------------------------------------+|            
              ...
              |                   |                   |                   
~~~~~~
{: #fig_dtsiot-flow_mesg title='Message flow for an SCHC session in a DtS-IoT environment.'}
...
## SCHC in ZE-Devices based on cellular
...
## SCHC for Low Power Wide Area (LPWA) Devices 
...
#### Device-initiated transmissions
 ...
 The figure {{fig_schc_over_dtsiot_dev_init_tx}} shows the transmission of a SCHC window in an uplink communication using the SCHC message flows defined in {{!RFC8724}}. The transmission of a SCHC window can be divided into two phases:


 * __Phase 1:__ In the visibility window, the end device sends the tiles to the LEO satellite. The tiles are carried by SCHC regular fragments. The tiles are stored in the LEO satellite. When the SCHC Proxy receives all tiles of a SCHC window, it sends to the end device a SCHC ACK. If the end device detects the loss of a tile(s) and the remaining visibility window time allows the tiles to be sent, the end device immediately retransmits the lost tile(s) without waiting for another visibility window.
  
 * __Phase 2:__ The LEO satellite sends the tiles stored in phase 1 to the SCHC gateway. The SCHC gateway receives the tiles and responds to the end device with an SCHC ACK message.
 ...
 Visibility |  |<--ACK, W=0, C=0 --| Bitmap:1101       | 
    ...
    Revisit |  |                   |--- W=0,FCN=1 ---->| 
       Time |  |                   |--- W=0,FCN=0 ---->| 
            ...
            v  |+-------------------------------------+| 
~~~~~~
{: #fig_schc_over_dtsiot_dev_init_tx title='SCHC over DtS-IoT '}


### SCHC with FEC mechanism

In this mechanism, the end-device (fragmenter) and the SCHC gateway (reassembler) use a Forward Error Correction mechanism (FEC) to protect the tiles. In this mechanism, the successful transmission of tiles MAY be confirmed by a SCHC ACK message. Figure {{fig_schc_over_dtsiot_fec}} shows the transmission of a SCHC session with FEC mechanism in a DtS-IoT environment. More details over the implementation in draft-munoz-schc-over-dts-iot
    ...
    Revisit |  |                   |- Encoded Tiles -->|          
       Time |  |                   |- Encoded Tiles -->|          
            ...
            v  |+-------------------------------------+|          
               ...
               |                   |                   |
~~~~~~
{: #fig_schc_over_dtsiot_fec title='Transmission of an SCHC session using an FEC mechanism'}
